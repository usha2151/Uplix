...
const AdminAddFestival = () => {
  ...
  const [showMessageModal, setShowMessageModal] = useState(false); // State to manage message modal visibility
...
const emailScheduled = async (id) => {
 
    setFestivalId(id);
    try {
        const response = await axios.get(`http://localhost:8080/festivals/fetchscheduleData/${id}`);
        
        if (response.data) {
           
          setMessageSubject(response.data[0].festival_subject);
          setMessageContent(response.data[0].festival_message);
        }
      } catch (error) {
  ...
  const handleAddFestival = async (event) => {
    ...
    dispatch(AddFestivals(festivalData, 'admin'));
  ...
  const handleSendMessage = async (event) => {
    ...
    const emailData = {
        ...
        festival_subject: messageSubject,
        festival_message: messageContent,
      };
  ...
  return (
    <>
      <div className="w-full py-3 pl-7 pr-5 grid xl:grid-cols-12 lg:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-5 justify-start">
              ...
              <tbody>
                {currentItems.map((data, index) => (
                  ...
                  >
                    ...
                    <td className="py-4 px-1 text-sm font-normal text-[#637381]">
                      <button onClick={() => emailScheduled(data.festival_id)} className="text-blue-600 hover:text-blue-900">
        ...
        {showMessageModal && (
          <div className="fixed top-0 left-0 right-0 bottom-0 flex items-center justify-center bg-dark bg-opacity-50 z-50">
            <div className="bg-white p-5 rounded-lg relative" style={{ width: '50vw', maxWidth: '450px', height: 'auto', padding: '20px' }}>
              {/* Close Icon */}
              <button className="absolute top-0 right-0 mt-2 mr-2 text-gray-400 hover:text-gray-600" onClick={() => setShowMessageModal(false)}>
                <i className="fa-solid fa-xmark"></i>
              </button>
              ...
              <form onSubmit={handleSendMessage}>
                <div className="mb-4.5">
                  ...
                  <input
                    ...
                    className="w-full rounded h-12 border-[1.5px] border-stroke bg-transparent py-2 px-5 text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary"
                    required
                ...
                <div className="mb-4.5">
                  ...
                  <textarea
                    ...
                    className="w-full h-48 rounded border-[1.5px] border-stroke bg-transparent py-2 px-5 text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary"
                    required
